...
public class Vocations {
    ...
    public static final Vocation TRAVELER = register(new VocationBuilder("traveler", NONE).addAttributeModifier(EntityAttributes.GENERIC_MOVEMENT_SPEED, "speed_modifier", 0.1, Operation.MULTIPLY_TOTAL).build());
    public static final Vocation BUILDER = createAndRegisterVocation("builder", NONE);
    public static final Vocation FIGHTER = register(new VocationBuilder("fighter", NONE).addAttributeModifier(EntityAttributes.GENERIC_ATTACK_DAMAGE, "speed_modifier", 1, Operation.ADDITION).build());
    ...
    private static Vocation createAndRegisterVocation(String name, Vocation parent) {
        Vocation vocation = new Vocation(name, parent);
